...
> **Trạng thái:** Chat/moderation đã triển khai; Community Video Events MVP đã được bổ sung với LiveKit, đăng ký, chat realtime trong cuộc họp và reminder
...
- Pharmacist/admin có thể làm host, bật/tắt trạng thái live, quản lý người tham gia và chat trong buổi hội thảo.
...
- FE đã có trang danh sách hội thảo, chi tiết hội thảo, LiveKit room, chat cuộc họp và admin management page.
...
- Trao đổi trực tiếp trong chat hội thảo theo chính sách kiểm duyệt community message.
...
- Phản hồi trực tiếp trong cuộc gọi/chat và ẩn nội dung không phù hợp trong chat hội thảo.
...
### 4.13 Chat trong hội thảo

Hội thảo MEDISPACE dùng chat realtime hiện có của `communityMessages` trong room. Không tạo luồng Q&A riêng vì mục tiêu sản phẩm là giải đáp và trao đổi ngay trong cuộc họp, giống trải nghiệm Google Meet.

Khuyến nghị cho MEDISPACE:

- Người tham gia nhắn tin trực tiếp trong panel `Chat cuộc họp` sau khi join LiveKit room.
- Tin nhắn đi qua moderation hiện có của community message.
- Host/dược sĩ phản hồi trực tiếp trong cuộc gọi hoặc trong chat, không cần quy trình duyệt chờ xử lý.
...
Chat hội thảo dùng lại `communityMessages` của room liên quan. Không có collection `communityVideoEventQuestions` trong thiết kế hiện tại.
...
| `GET` | `/rooms/:roomId/messages` | User đủ quyền room | Danh sách chat dùng trong hội thảo |
| `POST` | `/rooms/:roomId/messages` | User đủ quyền room | Gửi tin nhắn chat trong hội thảo |
...
| `community:video-event:{eventId}` | Kênh realtime của hội thảo video |
...
- Trạng thái hội thảo video, chat cuộc họp, attendee count và thông báo live.
...
| `AdminCommunityVideoEventsPage` | Quản lý hội thảo video: tạo link/lịch, cập nhật, start/end/cancel, xem đăng ký và hướng dẫn chat trực tiếp |
...
| Community video event detail | Xem thông tin hội thảo, đăng ký, join video, chat trực tiếp trong cuộc họp |
...
- Host UI có attendee count, start/end controls và chat trực tiếp trong cuộc họp.
- Hiển thị disclaimer y tế trước khi user join.
...
- Chat hội thảo đi qua moderation community message; host/dược sĩ phản hồi trực tiếp trong cuộc gọi hoặc chat.
...
- User join event và gửi tin nhắn trong chat cuộc họp.
- Host phản hồi trực tiếp trong cuộc gọi/chat.
...
- Số tin nhắn chat hội thảo visible/hidden/deleted.
...
- Tin nhắn chat bị ẩn nhiều bất thường trong lúc event đang live.
...
- Chat có moderation theo community message.
- Disclaimer y tế trước khi tham gia.
...
- Retention policy cho recording, attendance và chat message.
...
10. Bổ sung dashboard metric riêng cho attendance, no-show, meeting chat và lỗi join token.
