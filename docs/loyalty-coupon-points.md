...
> **Trạng thái:** Đã triển khai coupon/points/order/refund và loyalty program config draft/publish trên nhánh `feature/loyalty-coupon-points-fixes`
...
- Loyalty:
  ...
  - Cấu hình loyalty program bằng DB config versioned.
  - Admin lưu nháp và publish cấu hình loyalty.
  - Earn/redeem/tier calculation đọc published config, fallback default nếu DB chưa có config.
- Order/refund:
...
- Points expiry policy nâng cao như notification trước khi hết hạn và grace period.
...
- Sửa cấu hình chương trình loyalty bằng bản nháp.
- Publish loyalty config để áp dụng cho earn/redeem/tier mới.
...
- Sửa published loyalty config trực tiếp. Published config chỉ nên thay bằng bản nháp mới được publish.
...
### 5.8 `loyalty_program_configs`

Cấu hình chương trình loyalty theo version. BE luôn đọc bản `published` mới nhất; nếu collection chưa có dữ liệu, hệ thống fallback default từ env/constants hiện tại để không làm gián đoạn production.

| Trường | Kiểu | Ý nghĩa |
|--------|------|---------|
| `_id` | ObjectId | ID config |
| `version` | number | Version tăng dần |
| `status` | `draft` / `published` / `archived` | Trạng thái config |
| `pointsPerVnd` | number | Bao nhiêu VNĐ tạo ra 1 điểm trước multiplier |
| `pointsToVnd` | number | 1 điểm quy đổi ra bao nhiêu VNĐ khi redeem |
| `maxRedeemRatio` | number | Tỷ lệ tối đa được đổi trên subtotal sau coupon, ví dụ `0.3` |
| `minRedeem` | number | Số điểm tối thiểu để được redeem |
| `expiryDays` | number | Số ngày điểm earn có hiệu lực |
| `tiers` | array | Danh sách hạng và rule |
| `createdBy` | ObjectId | Admin tạo draft |
| `updatedBy` | ObjectId | Admin cập nhật gần nhất |
| `publishedBy` | ObjectId | Admin publish |
| `publishedAt` | Date | Thời điểm publish |
| `createdAt` | Date | Thời điểm tạo |
| `updatedAt` | Date | Thời điểm cập nhật |

Rule tier trong `tiers`:

| Trường | Kiểu | Ý nghĩa |
|--------|------|---------|
| `code` | `member` / `silver` / `gold` / `platinum` | Mã hạng |
| `label` | string | Tên hiển thị |
| `minTotalSpent` | number | Tổng chi tiêu tối thiểu để đạt hạng |
| `multiplier` | number | Hệ số nhân điểm khi earn |

Rule publish:

- Chỉ một config `published` active tại một thời điểm.
- Khi publish draft mới, config published cũ chuyển `archived`.
- Admin sửa trên bản `draft`, không sửa trực tiếp bản `published`.
- Earn/redeem/tier sau thời điểm publish dùng config mới.
...
| `GET` | `/admin/program-config` | Admin | Lấy published config, draft config và defaults |
| `PUT` | `/admin/program-config/draft` | Admin | Lưu bản nháp cấu hình loyalty |
| `POST` | `/admin/program-config/publish` | Admin | Publish bản nháp mới nhất |
...
### 7.0 Loyalty program config active

Mọi phép tính loyalty hiện đọc từ config active:

```text
activeConfig = latest loyalty_program_configs where status = published
```

Nếu DB chưa có config published:

```text
activeConfig = DEFAULT_LOYALTY_CONFIG
```

Các field đang ảnh hưởng trực tiếp nghiệp vụ:

- `pointsPerVnd`: tính base points khi earn.
- `pointsToVnd`: quy đổi points thành VNĐ khi redeem.
- `maxRedeemRatio`: cap số tiền được redeem theo subtotal.
- `minRedeem`: điểm tối thiểu để redeem.
- `expiryDays`: hạn sử dụng của transaction earn.
- `tiers[].minTotalSpent`: xét hạng.
- `tiers[].multiplier`: nhân điểm khi earn.
...
requestedRedeemAmount = requestedPoints * activeConfig.pointsToVnd
maxRedeemByRatio = floor(remainingAfterCoupon * activeConfig.maxRedeemRatio)
pointsRedeemAmount = min(requestedRedeemAmount, balance * activeConfig.pointsToVnd, maxRedeemByRatio)
```
...
```text
basePoints = floor(orderTotal / activeConfig.pointsPerVnd)
tierMultiplier = activeConfig.tiers[currentTier].multiplier
earnedPoints = floor(basePoints * tierMultiplier)
expiresAt = now + activeConfig.expiryDays
```

Sau khi earn:

```text
newTotalSpent = oldTotalSpent + orderTotal
newTier = highest tier where newTotalSpent >= tier.minTotalSpent
```

### 7.7 Allocation coupon theo item
...
### 7.8 Allocation points theo item
...
| `LoyaltyManagementPage` | Xem account, adjustment points, cấu hình loyalty program draft/publish |
...
- Loyalty config phải dùng `Lưu nháp` trước, sau đó `Publish` để áp dụng.
- Published config đang áp dụng phải hiển thị version và rule chính để admin biết hệ thống đang chạy theo cấu hình nào.
...
| `loyalty_program_configs` | `status`, `version` |
| `loyalty_program_configs` | `version` unique |
...
- Loyalty preview redeem dùng published config thay vì hard-code env.
- Loyalty publish draft archive published config cũ và publish draft mới.
...
7. Admin sửa loyalty config, lưu nháp, reload vẫn thấy draft.
8. Admin publish draft, config mới ảnh hưởng preview redeem/earn order mới.
...
- BE test: 18 files, 338 tests passed.
- BE build: passed.
- FE typecheck: passed.
...
- Loyalty tier/rate/redeem/expiry được quản lý bằng `loyalty_program_configs`.
- Admin sửa loyalty program qua draft và publish, không sửa trực tiếp published config.
- BE đọc published config mới nhất và fallback default nếu DB chưa có config.
...
- Điểm đã có `expiryDays` trong config và đang expire theo transaction earn.
- Notification trước khi hết hạn chưa có.
- Grace period sau khi hết hạn chưa có.
...
- Kiểm tra admin publish loyalty config trên staging.
- Kiểm tra preview redeem/earn order mới sau khi publish loyalty config.
...
- Loyalty config mới chỉ ảnh hưởng order/transaction mới; order cũ vẫn phải dựa trên snapshot transaction/order.
- Nếu chưa có published config trong DB, BE fallback default để tránh downtime nhưng admin nên publish config chính thức sớm.
...
- `src/models/schemas/LoyaltyProgramConfig.schema.ts`
...
3. Snapshot loyalty config version vào order/transaction để audit chính xác rule tại thời điểm phát sinh.
4. Playwright E2E cho coupon/points/refund và loyalty config publish.
5. Dashboard audit cho coupon usage và loyalty transactions.
6. Alert/monitor cho coupon abuse và loyalty balance anomaly.
